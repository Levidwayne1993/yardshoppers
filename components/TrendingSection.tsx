...
export default function TrendingSection() {
  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      // ── FIX: Filter out expired sales — never show "Sale Ended" in Trending ──
      const today = new Date().toISOString().split("T")[0];
      const filterActive = (list: any[]) =>
        list.filter((l) => !l.sale_date || l.sale_date >= today);

      setTrending(filterActive(trendingData));
      setDrops(filterActive(dropsData));

      if (lat && lng) {
        const hotData = await getHotNearby(lat, lng, 50, 12);
        setHot(filterActive(hotData));
      }
    ...
    <section className="mt-12">
      ...
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-xl font-bold text-gray-900">
          <i
            className="fa-solid fa-chart-line text-ys-600 mr-2 text-lg"
            aria-hidden="true"
          />
          Real-Time Signals
        </h2>
                ...
                <div key={listing.id} className="relative">
                  {activeTab === "trending" && listing.view_count > 0 && (
                    <div className="absolute top-3 right-3 z-10 bg-orange-500 text-white text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1">
                      <i
                        className="fa-solid fa-fire text-[8px]"
                        aria-hidden="true"
                      />
                  ...
                  {activeTab === "drops" && (
                    <div className="absolute top-3 right-3 z-10 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1">
                      <i
                        className="fa-solid fa-arrow-down text-[8px]"
                        aria-hidden="true"
                      />
                      Price Drop
                    </div>
                  ...
                  {activeTab === "hot" && (
                    <div className="absolute top-3 right-3 z-10 bg-ys-700 text-white text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1">
                      <i
                        className="fa-solid fa-location-dot text-[8px]"
                        aria-hidden="true"
                      />
                      Nearby
                    </div>
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-8 h-8 bg-ys-50 rounded-lg flex items-center justify-center shrink-0 group-hover:bg-ys-100 transition">
                  <i
                    className="fa-solid fa-magnifying-glass text-xs text-ys-600"
                    aria-hidden="true"
                  />
                </div>
